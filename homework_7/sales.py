...
ship_mode = {"Standard": sum(float(row[4]) for row in rows if row[2] == "Standard"),
             "Second": sum(float(row[4]) for row in rows if row[2] == "Second"),
             "First": sum(float(row[4]) for row in rows if row[2] == "First")}
print(f'{"Ship Mode":<10}{"Total Sales":>15}')
print("-" * 25)
for mode, total_sales in ship_mode.items():
    print(f'{mode:<10}{total_sales:>15.2f}')
